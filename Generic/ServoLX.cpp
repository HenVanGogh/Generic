 ...
 * Read angles seems to match write angles
 */
#include "ServoLX.h"
#include <Arduino.h>
#include <stdint.h>
...
int ServoLX::temp(uint8_t which)
{
	return 0;
}

int ServoLX::volts(uint8_t which)
{
	return 0;
}
...
{
	//send(SERVOLX_BROADCAST, SERVOLX_ID_WRITE, &which, 1); safety issues
}
